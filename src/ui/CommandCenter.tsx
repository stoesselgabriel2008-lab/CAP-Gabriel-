...
export function CommandCenter({ onClose }: { onClose: () => void }) {
  ...
  const commands = useMemo<Cmd[]>(() => {
    ...
    const list: Cmd[] = [
      ...
      { id: 'a-resist', group: 'Actions', title: 'Compteurs de résistance', icon: 'flag', keywords: 'resister tenir craquer compteur jours serie abstinence arreter engagement discipline volonte streak record palier', run: go(() => ui.navigate('coach', 'resist')) },      { id: 'a-checkin', group: 'Actions', title: 'Faire un check-in', icon: 'bolt', keywords: 'humeur energie forme etat moral fatigue stress bilan matin comment ca va', run: go(() => ui.openCheckIn()) },
