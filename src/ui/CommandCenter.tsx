...
export function CommandCenter({ onClose }: { onClose: () => void }) {
  ...
  const commands = useMemo<Cmd[]>(() => {
    ...
    const list: Cmd[] = [
      ...
      { id: 'a-resist', group: 'Actions', title: 'Compteurs de résistance', icon: 'flag', keywords: 'resister tenir craquer compteur jours serie abstinence arreter engagement discipline volonte streak record palier', run: go(() => ui.navigate('coach', 'resist')) },
      { id: 'a-ai', group: 'Actions', title: 'Coach IA (Claude)', icon: 'mind', keywords: 'ia intelligence artificielle claude chat parler discuter conseil assistant coach question aide gpt bot', run: go(() => ui.navigate('coach', 'ai')) },
